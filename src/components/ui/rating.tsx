...
function RatingItem({ size, variant = 'default', value, point, hoveredValue, name, readOnly = false, disabled = false, precision, Icon, onMouseLeave, onValueChange, onValueHover, className, ...props }: RatingItemProps) {
  ...
  const icons = React.useMemo(() => {
    const emptyIcon = React.cloneElement(Icon, {
      size,
      className: cn('fill-muted-foreground/20 stroke-muted-foreground/10', variant === 'yellow' && 'fill-amber-600/30 stroke-amber-600/10 dark:fill-amber-400/30 dark:stroke-amber-400/10'),
      'aria-hidden': 'true',
    })
  ...
  return (
    <>
      <Comp
        ...
        className={cn('[&_svg]:pointer-events-none', isPartialPoint && 'pointer-events-none absolute top-0 left-0 overflow-hidden', isInteractive && 'cursor-pointer hover:scale-105', disabled && 'cursor-not-allowed opacity-50', className)}
      ...
      {!readOnly && <input type='radio' id={`${ratingIconId}-${point}`} name={name} value={point} className='sr-only' tabIndex={-1} data-slot='rating-input' />}
    </>
  )
}
...
function Rating({
  ...
  return (
    <div
      ...
      data-disabled={disabled}
      data-readonly={readOnly}
      className={cn('focus-visible:ring-ring/50 flex gap-px focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none', disabled && 'opacity-50', className)}
    ...
    >
      {stars.map(({ key, points }) => (
        <span key={key} data-slot='rating-star' className={cn('relative', isInteractive && 'transition-transform hover:scale-110', disabled && 'cursor-not-allowed')} aria-disabled={disabled} aria-hidden={readOnly}>
          {points.map((point) => (
            <RatingItem
              key={point}
              name={ratingName}
